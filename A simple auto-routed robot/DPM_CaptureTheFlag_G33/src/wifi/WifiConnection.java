...
public class WifiConnection {
	...
	public WifiConnection(String serverIP, int teamNumber) throws IOException {
		...
		Log.log(Log.Sender.init,"Opening wifi connection to server at IP: " + serverIP);
	    Socket socketClient = new Socket(serverIP, port);
	    LCD.drawString("Connected to server", 0, 1);
	    Log.log(Log.Sender.init,"Connected to server");
		...
		Log.log(Log.Sender.init,"Waiting from transmission...");
		
		while(dis.available() <= 0)
			try {Thread.sleep(10);} catch (InterruptedException e) {}
		LCD.drawString("Receiving transmission", 0, 3);	
		 Log.log(Log.Sender.init,"Receiving transmission");
		
		// Parse transmission
		this.trans = ParseTransmission.parse(dis);
		LCD.drawString("Finished parsing", 0, 4);
		 Log.log(Log.Sender.init,"Finished parsing");
		...
		LCD.drawString("Connection terminated", 0, 5);
		 Log.log(Log.Sender.init,"Connection terminated");
	...
	public void printTransmission() {
		try {
			...
			Log.log(Log.Sender.init,"Trans. Values");
			Log.log(Log.Sender.init,"Start: " + trans.startingCorner.toString());
			Log.log(Log.Sender.init,"HZ: " + trans.homeZoneBL_X + " " + trans.homeZoneBL_Y + " " + trans.homeZoneTR_X + " " + trans.homeZoneTR_Y);
			Log.log(Log.Sender.init,"OHZ: " + trans.opponentHomeZoneBL_X + " " + trans.opponentHomeZoneBL_Y + " " + trans.opponentHomeZoneTR_X + " " + trans.opponentHomeZoneTR_Y);
			Log.log(Log.Sender.init,"DZ: " + trans.dropZone_X + " " + trans.dropZone_Y);
			Log.log(Log.Sender.init,"Flg: " + trans.flagType + " " + trans.opponentFlagType);
		} catch (NullPointerException e) {
			LCD.drawString("Bad Trans", 0, 7);
			Log.log(Log.Sender.init,"Bad Trans");
		}
	}
